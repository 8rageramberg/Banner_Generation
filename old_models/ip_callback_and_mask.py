...
def main():
    ...
    grid_adapter = [1.0, 1.5, 0.3, 0.5]
    grid_guidance = [7.0, 9.0]
    grid_cn = [1.0, 0.9, 0.8]
    ...
    # --- after you’ve created `logo_mask` ---
    logo_mask = create_ip_adapter_mask(control_image, height, width, blurred=True, blur_padding=20)

    # 1) View / save the full-res mask
    logo_mask.save("debug_logo_mask.png")
    logo_mask.show()  # pops up a window (if your environment supports it)

    # ip_adapter_masks is a list of torch tensors of shape (1, H', W')
    # Grab the first mask, convert to NumPy, and plot:
    mask_tensor = ip_adapter_masks[0][0]       # remove batch dim
    mask_np = mask_tensor.cpu().numpy()
    plt.imshow(mask_np, cmap="gray")
    plt.axis("off")
    plt.title("IP-Adapter Mask (downsampled)")
    plt.show()
