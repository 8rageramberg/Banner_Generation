...
def dynamic_adapter_callback_creator_10(adapter_scale_default, steps):
    """
    Returns a callback function that updates the IP-adapter scale dynamically.
    The adapter scale remains at adapter_scale_default for the first 90% of steps,
    then decays linearly to 0 over the last 10% of the total steps.
    """
    def dynamic_adapter_callback(pipe_obj, step: int, timestep: float, callback_kwargs, **kwargs):
        # For the first 90% of steps, keep adapter scale at full value.
        if step < 0.9 * steps:
            new_scale = adapter_scale_default
        else:
            # In the final 90% of steps, decay linearly to 0.
            # The fraction of progress in the final 10% is: (steps - step) / (0.9 * steps)
            new_scale = adapter_scale_default * ((steps - step) / (0.9 * steps))
        pipe_obj.set_ip_adapter_scale(new_scale)
        print(f"Step {step}/{steps}: Adapter scale = {new_scale:.3f}")
        # Return an empty dict to satisfy pipeline expectations.
        return {}
...
def run_grid_search(pipe, prompt, negative_prompt, steps, grid_guidance, grid_adapter, grid_cn,
                    image_embeds, control_image, seed, device, grid_dir, height, width):
    """
    ...
    for g, a, cn in itertools.product(grid_guidance, grid_adapter, grid_cn):
        ...
        dynamic_callback = dynamic_adapter_callback_creator_10(a, steps)
...
def main():
    ...
    results_info = run_grid_search(
        ...
        image_embeds, control_image, seed, device,
        grid_dir, height, width
    )
